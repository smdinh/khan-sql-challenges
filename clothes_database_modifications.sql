/** Challenge: Clothing alterations
Lesson 4: Modifying databases with SQL
We've created a database of clothes, and decided we need a price column.
Use ALTER to add a 'price' column to the table. Then select all the columns in each row to see what your table looks like now.
Now assign each item a price, using UPDATE - item 1 should be 10 dollars, item 2 should be 20 dollars, item 3 should be 30 dollars.
When you're done, do another SELECT of all the rows to check that it worked as expected.
Now insert a new item into the table that has all three attributes filled in, including 'price'. Do one final SELECT of all the rows to check it worked.
**/

-- code provided by Khan Academy
CREATE TABLE clothes
(
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    type TEXT,
    design TEXT
);

INSERT INTO clothes
(
    type,
    design
)
VALUES
("dress", "pink polka dots");
INSERT INTO clothes
(
    type,
    design
)
VALUES
("pants", "rainbow tie-dye");
INSERT INTO clothes
(
    type,
    design
)
VALUES
("blazer", "black sequin");

-- Step 1: Use ALTER to add a 'price' column to the table
ALTER TABLE clothes ADD price REAL;
-- Select all the columns in each row to see what your table looks like now
SELECT *
FROM clothes;

-- Step 2: Assign each item a price, using UPDATE - item 1 should be 10 dollars, item 2 should be 20 dollars, item 3 should be 30 dollars
UPDATE clothes
SET price = 10
WHERE id = 1;
UPDATE clothes
SET price = 20
WHERE id = 2;
UPDATE clothes
SET price = 30
WHERE id = 3;
-- SELECT of all the rows to check that it worked as expected
SELECT *
FROM clothes;

-- Step 3: Insert a new item into the table that has all three attributes filled in
INSERT INTO clothes
(
    type,
    design,
    price
)
VALUES
("pants", "loose fit denim", 49.99);
-- SELECT of all the rows to check that it worked as expected
SELECT *
FROM clothes;
