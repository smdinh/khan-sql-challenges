...
-- Step 1: Select all of the rows, and display the name, number_grade, and percent_completed
SELECT name,
       number_grade,
       ROUND(fraction_completed * 100) AS percent_completed
FROM student_grades;

-- Step 2: Query a table that shows how many students have earned which letter_grade
SELECT COUNT(*),
       CASE
           WHEN number_grade > 90 THEN
               "A"
           WHEN number_grade > 80 THEN
               "B"
           WHEN number_grade > 70 THEN
               "C"
           WHEN number_grade <= 70 THEN
               "F"
       END as "letter_grade"
